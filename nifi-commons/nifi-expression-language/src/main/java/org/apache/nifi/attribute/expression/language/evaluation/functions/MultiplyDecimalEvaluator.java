...
public class MultiplyDecimalEvaluator extends DecimalEvaluator {

    private final Evaluator<Double> subject;
    private final Evaluator<Double> multiplyValue;

    public MultiplyDecimalEvaluator(final Evaluator<Double> subject, final Evaluator<Double> multiplyValue) {
    ...
    public QueryResult<Double> evaluate(final Map<String, String> attributes) {
        ...
        final Double multiply = multiplyValue.evaluate(attributes).getValue();
        if (multiply == null) {
            return new DecimalQueryResult(null);
        }

        final double result = subjectValue * multiply;
