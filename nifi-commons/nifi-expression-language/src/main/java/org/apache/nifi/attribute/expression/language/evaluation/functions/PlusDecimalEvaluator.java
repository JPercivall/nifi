...
public class PlusDecimalEvaluator extends DecimalEvaluator {

    private final Evaluator<Double> subject;
    private final Evaluator<Double> plusValue;

    public PlusDecimalEvaluator(final Evaluator<Double> subject, final Evaluator<Double> plusValue) {
    ...
    public QueryResult<Double> evaluate(final Map<String, String> attributes) {
        ...
        final Double plus = plusValue.evaluate(attributes).getValue();
        if (plus == null) {
            return new DecimalQueryResult(null);
        }

        final double result = subjectValue + plus;
