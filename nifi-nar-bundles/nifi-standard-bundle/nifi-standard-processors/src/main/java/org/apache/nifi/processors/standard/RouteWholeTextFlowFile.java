...
public class RouteWholeTextFlowFile extends AbstractProcessor {
    ...
    private static final String routeFlowFileWhenAnyLinesMatchValue = "Route when any line matches the match strategy.";
    private static final String routeFlowFileWhenAllLinesMatchValue = "Route when all lines match the match strategy";
    ...
    public static final AllowableValue ROUTE_FLOW_FILE_WHEN_ANY_LINES_MATCH = new AllowableValue(routeFlowFileWhenAnyLinesMatchValue, routeFlowFileWhenAnyLinesMatchValue,
            "The whole FlowFile will be routed as one. When determining a match, if any line successfully matches the 'Match Strategy' then the FlowFile will be transferred "
                    + "to the corresponding relationship");
    public static final AllowableValue ROUTE_FLOW_FILE_WHEN_ALL_LINES_MATCH = new AllowableValue(routeFlowFileWhenAllLinesMatchValue, routeFlowFileWhenAllLinesMatchValue,
            "The whole FlowFile will be routed as one. When determining a match, every line must successfully match the 'Match Strategy' in order for the FlowFile to be transferred "
                    + "to the corresponding relationship");

    // TODO: nail down name
    public static final PropertyDescriptor FLOWFILE_ROUTING = new PropertyDescriptor.Builder()
            .name("Lines Required to Route")
            .description("Indicates whether FlowFile should be routed when either all or any lines are accepted by the match strategy.")
            .required(true)
            .allowableValues(ROUTE_FLOW_FILE_WHEN_ANY_LINES_MATCH, ROUTE_FLOW_FILE_WHEN_ALL_LINES_MATCH)
            .defaultValue(ROUTE_FLOW_FILE_WHEN_ALL_LINES_MATCH.getValue())
            .dynamic(false)
            .build();
    ...
    protected void init(final ProcessorInitializationContext context) {
        ...
        properties.add(FLOWFILE_ROUTING);
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) {
        ...
        final String flowFileRoutingDecision = context.getProperty(FLOWFILE_ROUTING).getValue();
        ...
        for (final Map.Entry<Relationship, PropertyValue> entry : propMap.entrySet()) {
            ...
            if(compileRegex){
                compiledRegex = Pattern.compile(value);
            }
            propValueMap.put(entry.getKey(), compileRegex ? compiledRegex : value);
        }

        final Set<Relationship> routingRelationshipsTemp = new HashSet<>();
                ...
                try (final Reader inReader = new InputStreamReader(in,charset);
                    ...
                    while ((line = reader.readLine()) != null) {
                        ...
                        for (final Map.Entry<Relationship, Object> entry : propValueMap.entrySet()) {
                            ...
                            if(routingRelationshipsTemp.contains(entry.getKey())){
                                continue;
                            }

                            String matchLine = trim ? line.trim() : line;
                            boolean lineMatchesProperty = lineMatches(matchLine, entry.getValue(), context.getProperty(MATCH_STRATEGY).getValue());
                            if (lineMatchesProperty) {
                                propertiesThatMatchedLine++;
                            }

                            // 1
                            if (!lineMatchesProperty && routePropertyNameValue.equals(routeStrategy) && routeFlowFileWhenAllLinesMatchValue.equals(flowFileRoutingDecision)) {
                                //can't
                                routingRelationshipsTemp.add(entry.getKey());
                                continue;
                            }

                            // 2
                            if (lineMatchesProperty && routePropertyNameValue.equals(routeStrategy) && routeFlowFileWhenAnyLinesMatchValue.equals(flowFileRoutingDecision)) {
                                //to route to
                                routingRelationshipsTemp.add(entry.getKey());
                                continue;
                            }
                            ...
                            // 3
                            if(!lineMatchesProperty && routeAllMatchValue.equals(routeStrategy) && routeFlowFileWhenAllLinesMatchValue.equals(flowFileRoutingDecision)){
                                // to route to
                                routingRelationshipsTemp.add(REL_NO_MATCH);
                                return;
                            }

                            // 4
                            if(!lineMatchesProperty && routeAllMatchValue.equals(routeStrategy) && routeFlowFileWhenAnyLinesMatchValue.equals(flowFileRoutingDecision)){
                                break;
                            }

                            // break as soon as possible to avoid calculating things we don't need to calculate.
                            if (lineMatchesProperty && routeAnyMatchValue.equals(routeStrategy)) {
                                break;
                            }

                        }
                        // 4
                        if (propertiesThatMatchedLine > 0 && routeAllMatchValue.equals(routeStrategy) && routeFlowFileWhenAnyLinesMatchValue.equals(flowFileRoutingDecision)) {
                            //to route to
                            routingRelationshipsTemp.add(REL_MATCH);
                            return;
                        }

                        // 5
                        if (propertiesThatMatchedLine == 0 && routeAnyMatchValue.equals(routeStrategy) && routeFlowFileWhenAllLinesMatchValue.equals(flowFileRoutingDecision)) {
                            // to route to
                            routingRelationshipsTemp.add(REL_NO_MATCH);
                            return;
                        }
                        ...
                        // 6
                        if (propertiesThatMatchedLine !=0 && routeAnyMatchValue.equals(routeStrategy) && routeFlowFileWhenAnyLinesMatchValue.equals(flowFileRoutingDecision)) {
                            //to route to
                            routingRelationshipsTemp.add(REL_MATCH);
                            return;
                        }
        ...
        Set<Relationship> relationshipsToRouteTo = new HashSet<>();
        // 1
        if(routePropertyNameValue.equals(routeStrategy) && routeFlowFileWhenAllLinesMatchValue.equals(flowFileRoutingDecision)) {
            for (final Map.Entry<Relationship, Object> entry : propValueMap.entrySet()){
                if(!routingRelationshipsTemp.contains(entry.getKey())){
                    relationshipsToRouteTo.add(entry.getKey());
                }
            }
            if(relationshipsToRouteTo.isEmpty()){
                relationshipsToRouteTo.add(REL_NO_MATCH);
            }
            // 2
        } else if (routePropertyNameValue.equals(routeStrategy) && routeFlowFileWhenAnyLinesMatchValue.equals(flowFileRoutingDecision)){
            if(routingRelationshipsTemp.isEmpty()){
                relationshipsToRouteTo.add(REL_NO_MATCH);
            } else {
                relationshipsToRouteTo = routingRelationshipsTemp;
            }
            // 3
        }else if (routeAllMatchValue.equals(routeStrategy) && routeFlowFileWhenAllLinesMatchValue.equals(flowFileRoutingDecision)){
            if(routingRelationshipsTemp.isEmpty()){
                relationshipsToRouteTo.add(REL_MATCH);
            } else {
                relationshipsToRouteTo = routingRelationshipsTemp;
            }
            // 4
        } else if (routeAllMatchValue.equals(routeStrategy) && routeFlowFileWhenAnyLinesMatchValue.equals(flowFileRoutingDecision)){
            if(routingRelationshipsTemp.isEmpty()){
                relationshipsToRouteTo.add(REL_NO_MATCH);
            } else {
                relationshipsToRouteTo = routingRelationshipsTemp;
            ...
            // 5
        } else if (routeAnyMatchValue.equals(routeStrategy) && routeFlowFileWhenAllLinesMatchValue.equals(flowFileRoutingDecision)){
            if(routingRelationshipsTemp.isEmpty()){
                relationshipsToRouteTo.add(REL_MATCH);
            } else {
                relationshipsToRouteTo = routingRelationshipsTemp;
            ...
            // 6
        } else if (routeAnyMatchValue.equals(routeStrategy) && routeFlowFileWhenAnyLinesMatchValue.equals(flowFileRoutingDecision)) {
            if (routingRelationshipsTemp.isEmpty()) {
                relationshipsToRouteTo.add(REL_NO_MATCH);
            } else {
                relationshipsToRouteTo = routingRelationshipsTemp;
        ...
        for (final Relationship relationship : relationshipsToRouteTo) {
            FlowFile clone = session.clone(originalFlowFile);
            logger.info("Cloned {} into {} and routing clone to relationship {}", new Object[] {originalFlowFile, clone, relationship});
    ...
    protected static boolean lineMatches(final String line, final Object comparison, final String matchingStrategy) {
        switch (matchingStrategy) {
            ...
            case equalsValue:
                return line.equals((String) comparison);
